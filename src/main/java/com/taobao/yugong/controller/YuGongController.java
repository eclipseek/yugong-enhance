...
import com.taobao.yugong.common.db.meta.DatabaseMetaGenerator;
import com.taobao.yugong.common.model.*;
import com.taobao.yugong.translator.DefaultTranslator;
...
public class YuGongController extends AbstractYuGongLifeCycle {
    ...
    public void start() {
        ...
        this.translatorDir = new File(config.getString("yugong.translator.dir", "./yugong-master/conf/translator"));
    ...
    private DataTranslator getDefaultTranslator() {
    ...
    private DataSource initDataSource(String type) {
        ...
        String maxWait = config.getString("yugong.database." + type + ".maxWait");
    ...
    private Collection<TableHolder> initTables() {
        ...
        final List tableBlackList = config.getList("yugong.table.black");
...
//        }
        ...
        if (!isEmpty) {
            ...
            queue.addAll(tableWhiteList);
            List<Thread> threads = new ArrayList<Thread>();
            for(int i = 0; i < globalContext.getBuildThreadNum(); i++) {
                            ...
                            while(true) {
                                ...
                                if (obj == null) {
                                    ...
                                    if (!isBlackTable(whiteTable, tableBlackList)) {
                                        logger.info(val + " ---> build table info: " + whiteTable);
                                        ...
                                        logger.info(val + " ---> table: " + whiteTable + " in black list, ingore it.");
                                    }
                                }
                            }
                        } catch (InterruptedException e) {
            ...
            logger.info("main thread wait for all column build finish...");
            for (Thread thread: threads) {
                try {
                    thread.join();
                } catch (InterruptedException e) {
                    ...
                    public void run() {
                        try {
                            while(true) {
                                ...
                                Table table = queue.poll(1000, TimeUnit.MILLISECONDS);
                                if (table == null) {
                                    if (queue.isEmpty()) {
                                        break;
                                    }
                                    ...
                                    if (!isBlackTable(table.getName(), tableBlackList)
                                        ...
                                        logger.info(val + " ---> table: " + table.getName() + " in black list, ingore it.");
                                    }
                                }
                            }
                        } catch (InterruptedException e) {
